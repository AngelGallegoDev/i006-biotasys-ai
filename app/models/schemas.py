...
class GutHealthScore(BaseModel):
    ...
    label: str = Field(..., description="Qualitative label (e.g. 'Excelente', 'Necesita Mejorar')")
    breakdown: str = Field(..., description="Explanation of how the score was calculated")
...
class DietaryRecommendation(BaseModel):
    ...
    action: str = Field(..., description="Action to take: 'Aumentar', 'Reducir', 'Evitar'")
    reason: str = Field(..., description="Why this recommendation is made based on microbiota")


class SupplementSuggestion(BaseModel):
    """Supplement recommendation."""
    type: str = Field(..., description="Type: 'Probiótico', 'Prebiótico', 'Suplemento'")
    name: str = Field(..., description="Specific name (e.g. 'Lactobacillus rhamnosus GG')")
...
class MicrobiotaInterpretation(BaseModel):
    """Advanced technical report generated by Gemini."""
